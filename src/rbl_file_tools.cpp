...
{
    R_LOG_TRACE_IN;
    RLogger::info("FileService: Reading binary file \"%s\".\n",fileName.toUtf8().constData());
    ...
    try
    {
        ...
        if(!inFile.open(QIODevice::ReadOnly))
        {
            ...
            throw RError(RError::Type::OpenFile,R_ERROR_REF,
                         "Failed to open binary file \"%s\" for reading. %s.",
    ...
    {
        RLogger::error("FileService: Failed to read binary file \"%s\". %s\n", fileName.toUtf8().constData(),e.what());
    ...
    {
        RLogger::error("FileService: Failed to read binary file \"%s\".\n", fileName.toUtf8().constData());
        R_LOG_TRACE_RETURN(false);
    }
...
{
    R_LOG_TRACE_IN;
    RLogger::info("FileService: Writing binary file \"%s\".\n",fileName.toUtf8().constData());
...
bool RFileTools::readAsciiFile(const QString &fileName, QByteArray &byteArray)
{
    R_LOG_TRACE_IN;
    RLogger::info("FileService: Reading text file \"%s\".\n",fileName.toUtf8().constData());
    ...
    try
    {
        ...
        if(!inFile.open(QIODevice::ReadOnly | QIODevice::Text))
        {
            R_LOG_TRACE_OUT;
            throw RError(RError::Type::OpenFile,R_ERROR_REF,
                         "Failed to open text file \"%s\" for reading. %s.",
                         inFile.fileName().toUtf8().constData(),
                         inFile.errorString().toUtf8().constData());
        }
    ...
    {
        RLogger::error("FileService: Failed to read text file \"%s\". %s\n", fileName.toUtf8().constData(),e.what());
        R_LOG_TRACE_RETURN(false);
    }
    catch (const RError &e)
    {
    ...
    {
        RLogger::error("FileService: Failed to read text file \"%s\".\n", fileName.toUtf8().constData());
        R_LOG_TRACE_RETURN(false);
    }
    R_LOG_TRACE_RETURN(true);
}

bool RFileTools::writeAsciiFile(const QString &fileName, const QByteArray &byteArray)
{
    R_LOG_TRACE_IN;
    RLogger::info("FileService: Writing text file \"%s\".\n",fileName.toUtf8().constData());
    ...
    try
    {
        ...
        if(!outFile.open(QIODevice::WriteOnly | QIODevice::Text))
        {
            R_LOG_TRACE_OUT;
            throw RError(RError::Type::OpenFile,R_ERROR_REF,
                         "Failed to open text file \"%s\" for writing. %s.",
                         outFile.fileName().toUtf8().constData(),
                         outFile.errorString().toUtf8().constData());
        }
    ...
    {
        RLogger::error("FileService: Failed to write text file \"%s\". %s\n", fileName.toUtf8().constData(),e.what());
        R_LOG_TRACE_RETURN(false);
    }
    catch (const RError &e)
    {
    ...
    {
        RLogger::error("FileService: Failed to write text file \"%s\".\n", fileName.toUtf8().constData());
        R_LOG_TRACE_RETURN(false);
    }
    R_LOG_TRACE_RETURN(true);
}
