...
void RValueTable::_init(const RValueTable *pValueTable)
{
    if (pValueTable)
    {
        ...
        this->clear();
        for (uint i=0;i<pValueTable->size();i++)
        {
...
const QString &RValueTable::getKeyName() const
{
    return this->keyName;
...
const QString &RValueTable::getKeyUnits() const
{
...
const QString & RValueTable::getValueName() const
{
...
const QString &RValueTable::getValueUnits() const
{
...
uint RValueTable::size() const
{
    return (uint)this->table.size();
} /* RValueTable::size */
...
void RValueTable::remove (uint position)
{
...
double RValueTable::getKey (uint position) const
{
...
double RValueTable::getValue (uint position) const
{
    R_ERROR_ASSERT(position < this->table.size());
    std::map<double,double>::const_iterator iter = this->table.begin();
    std::advance (iter, position);
